import React from 'react';
import {PlusIcon} from '@heroicons/react/solid'
import Image from 'next/image'
interface StoryProps {
  username: string | null | undefined;
  img: string | null | undefined;
  currentUser?: boolean;
}

const Story = ({ username, img,currentUser }: StoryProps) => {
  return (
    <div className=" flex flex-col items-center justify-start group cursor-pointer h-20">
      <div className="w-10 h-10 relative border-2 box-border border-rose-500 rounded-full">
      <Image
        className="absolute rounded-full cursor-pointer group-hover:scale-105 transition-transform duration-200 ease-out "
        src={
          img ||
          'data:image/png;base64,'
        }
        width={41}
        height={41}
        objectFit="cover"
        alt={username || 'user profile'}
      />
      </div>
      {currentUser && <PlusIcon className='h-6 absolute top-[8px] left-4 text-yellow-50' />}
      <p className="text-xs w-14 truncate mt-[1px] pt-1 text-center">{username}</p>
      
    </div>
  );
};

export default Story;
