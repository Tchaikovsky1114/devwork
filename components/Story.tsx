import React from 'react';
import {PlusIcon} from '@heroicons/react/solid'
interface StoryProps {
  username: string | null | undefined;
  img: string | null | undefined;
  currentUser?: boolean;
}

const Story = ({ username, img,currentUser }: StoryProps) => {
  return (
    <div className=" relative flex flex-col items-center justify-start group cursor-pointer">
      <img
        className="h-14 rounded-full border-2 box-border border-red-500 cursor-pointer group-hover:scale-105 transition-transform duration-200 ease-out"
        src={
          img ||
          'data:image/png;base64,'
        }
        alt={username || 'user profile'}
      />
      {currentUser && <PlusIcon className='h-6 absolute top-4 left-4 text-yellow-50' />}
      <p className="text-xs w-14 truncate mt-[1px] pt-1 text-center">{username}</p>
    </div>
  );
};

export default Story;
